SELECT
	...
	END AS municipio_cliente_atendido
	, CASE 
	...
	, nr.DescNaturezaJuridica as descricao_natureza
	, pa.DescNaturezaPublicoAlvo as natureza_publico_alvo
	, si.DescSituacaoInstrumento as situacao_instrumento
	, pf.NuTelefoneClientePessoaFisica as telefone_pf
	, pf.EmailClientePessoaFisica as email_pf
	, pf.NomeClientePessoaFisica as nome_pf
FROM dbo.TFExecucaoAtendimento atend with (nolock)
...
LEFT JOIN dbo.TDAplicacaoInstrumento ai with (nolock) on (atend.SkAplicacaoInstrumento_Execucao = ai.SkAplicacaoInstrumento)
LEFT JOIN dbo.TDNaturezaJuridica nr with (nolock) on (atend.SkNaturezaJuridica = nr.SkNaturezaJuridica)
LEFT JOIN dbo.TDSituacaoInstrumento si with (nolock) on (atend.SkSituacaoInstrumento = si.SkSituacaoInstrumento)
WHERE
	(
		(atend.SkSebrae = 10 and atend.skppa = '{ppa_param}')
		OR (
			atend.skppa = '{ppa_param}'
			and atend.SkSebrae = 28
			and atend.SkClientePessoaJuridica > 0 
			and atend.SkPublicoAlvo in (3, 4, 5) 
			and atend.SkTipoPublicoAlvo = 1 
			and loc_pj.SiglaUF = 'MA'
		)
		OR (
			atend.skppa = '{ppa_param}'
			and atend.SkSebrae = 28
			and atend.SkClientePessoaFisicaAtendida > 0 
			and loc_pf.SiglaUF = 'MA'
		)
	)
	and atend.SkAceitabilidadeAtendimento = 15
	and atend.SkSituacaoInstrumento = 2
	and atend.SkTipoPublicoAlvo = 1